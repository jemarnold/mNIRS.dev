...
#'  no subsequent greater extremes within a trailing window in units of `x`
#'  defined by `end_kinetics_span`, or the limits of the data.
#' @param method Indicates which model to evaluate the kinetics event (see
#' *Details* for method parametrisation).
#'  \describe{
...
#'      to recover half of the total kinetics amplitude.}
...
process_kinetics.monoexponential <- function(
    ...
    intake <- pre_process_kinetics_names(y = y, x = x, data = data,
                                         x0 = x0, end_kinetics_span = end_kinetics_span,
                                         verbose = verbose)
    df <- intake$df
    data <- intake$data
    x_name <- names(data)[1]
...
#' Find the First Extreme Peak or Trough Value in a Numeric Vector
#'
#' Determines the direction of the response of `y` (positive or negative) and finds
#' the first extreme value (peak for positive, or trough for negative) in which no
#' greater extreme is observed within a trailing window in units of `x` defined by
#' `end_kinetics_span`, or the limits of the data.
#'
#' @inheritParams process_kinetics
#'
#' @seealso [process_kinetics()]
#'
#' @return A list with components:
#'      \item{`x`}{The filtered `x` values from the origin to the end of the
#'      trailing window defined by `end_kinetics_span` past the first extreme value.}
#'      \item{`y`}{The filtered `y` values from the origin to the end of the
#'      trailing window defined by `end_kinetics_span` past the first extreme value.}
#'      \item{`extreme`}{A [tibble][tibble::tibble-package] with the `x` and `y`
#'      coordinates of the first extreme value, as columns.}
#'
#' @keywords internal
#' @export
find_first_extreme <- function(y, x = NULL, end_kinetics_span = 15) {
    ...
    for (extreme_idx in extreme_indices) {
        ...
        window_end <- extreme_x + end_kinetics_span
