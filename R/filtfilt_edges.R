...
#'
#' Custom Butterworth filtering function from \code{\link{signal::butter()}} which handles data 'edges' better, e.g. the start
#' and end of data.
#'
...
#' @param edges Indicates how to pad `x`. *"rev"* (the default) will pad `x` with the preceding
#' 10% data in reverse sequence. *"rep1"* will pad `x` with the last preceding value. Otherwise
