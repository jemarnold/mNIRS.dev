...
#'
#' This function retrieves raw data from [read_data()] and applies processing methods:
#' 1) Removes outliers (not implemented) and custom fixed values such as c(0, 100).
#' 2) Handles missing data by either linear interpolation or omission.
#' 3) Applies digital filtering with either a Butterworth low-pass filter, simple moving average,
#' or a smoothing spline with default or custom parameters.
#' 4) Rescales the data by either shifting values to be positive, and/or normalising to
#' the observed range of data, either column-wise (each column scaled individually) or
#' dataframe-wise (all columns on the same scale).
#'
#' @param .data The data retrieved from [read_data()].
#' @param sample_rate An integer scalar indicating the sample rate at which the raw data was
#' recorded (in Hz; samples per second).
## #' @param remove_outliers A boolean NOT IMPLEMENTED.
#' @param remove_fixed_values (optional) Either `<NULL>` (the default) or a numeric vector of
#' values to remove from the data. e.g. `remove_fixed_values = c(0, 100)`.
#' @param handle_missing_values Indicates how to handle missing data.
#' - *"linear interpolate"* (the default) will interpolate between existing values.
#' - *"none"* will pass through missing data. This may introduce errors further
#' along in processing.
#' @param filter_method Indicates the digital filter method used to smooth data.
#' - *"none"* (the default) will pass through raw data without any digital filtering.
...
#' @param filter_parameters (optional) Either `<NULL<` or a named numeric vector of parameters
#' to pass through to the digital filter defined in `filter_method`.
#' - For `filter_method = "low-pass"` this should be `c(n, fc)` or `c(n, W)` where `n`
#' is an integer specifying the filter order number; `fc` is a numeric value specifying
#' the cutoff frequency in Hz; `W` is a numeric scalar between 0 and 1 specifying the
#' relative cutoff frequency, where 1 is the Nyquist frequency (half of the sample frequency
#' in Hz).
...
#' scaled from the starting value (e.g., Artinis Oxysoft). This does not change the scale of the
#' data.
#' @param normalise_range Indicates whether and how the range of observed mNIRS data should be
#' re-scaled to between 0 and 100. For example, to be used when calibrating the data to an
#' ischaemic occlusion (physiological range), or to the existing observed data (functional range).
#' - *"none"* (the default) will not normalise any data.
#' - *"column-wise"* will re-scale each NIRS column to 0-100 within it's own range.
#' - *"all"* will re-scale all NIRS columns to a common 0-100 range, from the
#' minimum to the maximum values across all columns.
#' - Alternatively, a list of character vectors of NIRS column names can be specified to
#' re-scale across selected columns in groups, e.g. for left and right limbs separately, such
#' as `c("smo2_left", "smo2_right")`.
...
process_data <- function(
    ...
    event_column <- attributes(.data)$event_column
    ...
    if (!(is.null(remove_fixed_values) | rlang::is_double(remove_fixed_values))) {
        cli::cli_abort(paste(
    ...
    if (!is.null(filter_parameters)) {
        if (filter_method == "moving-average" &
            ...
            cli::cli_abort(paste(
                "{.arg filter_parameters} should be a named {.cls numeric} vector",
                "which includes {.arg n} and one of either {.arg fc} or {.arg W}.",
                "Where {.arg n} specifies the low-pass filter order and",
                "{.arg fc} specifies the cutoff frequency in Hz, or",
                "{.arg W} specifies the relative cutoff frequency as a scalar between",
                "{.val 0} and {.val 1}, where {.val 1} is the Nyquist frequency",
                "(half of the sample frequency in Hz)."))
        } else if (filter_method == "smooth-spline" &
                   !any(grepl("spar", names(filter_parameters), ignore.case = TRUE))) {
            cli::cli_abort(paste(
                "{.arg filter_parameters} should be a named {.cls numeric} vector",
                "which includes {.arg spar}. Where {.arg spar} is a numeric scalar between",
                "{.val -1.5} and {.val 1.5} and specifies the smoothing parameter",
                "of the {.fn smooth.spline}."))
        }
    }
    ...
    if (!isTRUE(shift_range_positive) & !isFALSE(shift_range_positive)) {
        ...
        ## validation: warns that shift_range_positive is unnecessary if no negative or 0 values
    ...
    if (!all(unlist(normalise_range) %in%
             c("none", "column-wise", "all", names(nirs_columns), nirs_columns))) {
        cli::cli_abort(paste(
            "{.arg normalise_range} must be either {.val none}, {.val column-wise},",
            "{.val all}, or a list of names from {.arg nirs_columns}. Make sure column",
            "names match exactly"))
    }
    ...
    if (!is.null(remove_fixed_values)) {
        ...
        cli::cli_alert_info(paste(
            "Values {.val {remove_fixed_values}} will be removed from {.arg nirs_columns}",
            "and set to {.val {NA}}."
        ))
    ...
    if (is.null(filter_parameters) & filter_method == "low-pass") {
        ...
        cli::cli_alert_info(
            "Low-pass filter order cutoff frequency {.arg fc} set to {.val {fc}} Hz.")
    } else if (is.null(filter_parameters) & filter_method == "moving-average") {
        k <- 5

        cli::cli_alert_info(
            "Moving-average window {.arg k} set to {.val {k}} samples")
    } else if (is.null(filter_parameters) & filter_method == "smooth-spline") {
        ...
        spar_string <- paste(paste(names(spar), spar, sep = " = "), collapse = ", ")

        cli::cli_alert_info(paste(
            "smooth-spline {.arg spar}{cli::qty(names(spar))} parameter{?s} set:",
            "{.val {spar_string}}"))
    ...
    if (all(normalise_range == "column-wise")) {
        ...
        data_normalised <- data_shifted |>
            dplyr::mutate(
                dplyr::across(
                    dplyr::any_of(names(nirs_columns)),
                    ~ (. - min(., na.rm = TRUE)) / diff(range(., na.rm = TRUE))*100),
            )
        ...
        data_normalised <- data_shifted |>
            dplyr::mutate(
                dplyr::across(
                    dplyr::any_of(names(nirs_columns)),
                    ~ (. - min(dplyr::pick(dplyr::any_of(names(nirs_columns))), na.rm = TRUE)) /
                        diff(range(dplyr::pick(dplyr::any_of(names(nirs_columns))), na.rm = TRUE))*100),
            )
        ...
        data_normalised <-
            purrr::map(
                if (is.list(normalise_range)) normalise_range else list(normalise_range),
                \(norm_range)
                data_shifted |>
                    ...
                    dplyr::mutate(
                        dplyr::across(
                            ...
                            ~ (. - min(dplyr::pick(dplyr::any_of(norm_range)), na.rm = TRUE)) /
                                diff(range(dplyr::pick(dplyr::any_of(norm_range)), na.rm = TRUE))*100),
                    )
            ) |>
            dplyr::bind_cols(
                dplyr::select(data_shifted, -dplyr::any_of(unlist(normalise_range)))
            ) |>
    ...
    metadata <- list(
        ...
        # outliers_index = NA, ## TODO NOT IMPLEMENTED
...
# library(tidyverse)
# library(JAPackage)
# source("C:/R-Projects/mNIRS/R/read_data.R")
# source("C:/R-Projects/mNIRS/R/filtfilt_edges.R")
# source("C:/R-Projects/mNIRS/R/signif_trailing.R")
# source("C:/R-Projects/mNIRS/R/process_data.R")
#
# raw_data <- read_data(
