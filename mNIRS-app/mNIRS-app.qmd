...
raw_data <- reactive({
    ...
    data <- mNIRS::read_data(
        ...
        nirs_columns = string_to_named_vector("O2Hb_VL = 2, HHb_VL = 3"),
        sample_column = string_to_named_vector("sample = 1"),
...
nirs_data <- reactive({
    ...
    nirs_data <- raw_data |> 
        ...
        mutate(
            across(any_of(nirs_columns), \(.x) round(.x, 4))
        )
...
output$nirs_table <- DT::renderDT({
    ...
    nirs_columns <- names(attributes(nirs_data)$nirs_columns)
...
output$plot <- renderPlot({
    ...
    nirs_data <- nirs_data()
    nirs_columns <- names(attributes(nirs_data)$nirs_columns)
    sample_column <- names(attributes(nirs_data)$sample_column)
    event_column <- names(attributes(nirs_data)$event_column)
