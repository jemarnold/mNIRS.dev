...
We may need to adjust our data to normalise mNIRS signal values across channels, or between individuals or trials, etc. For example, we may want to set our mean baseline value to zero for all NIRS signals. Or we may want to compare signal kinetics (the rate of change or time course of a response) after rescaling to the same relative dynamic range.
...
plot(data_shifted) +
    ...
    geom_rect(aes(xmin = 0, xmax = 120, ymin = -5, ymax = 5),
...
plot(data_rescaled) +
    ...
    geom_rect(aes(xmin = 825, xmax = 975, ymin = 0, ymax = 100),
              colour = "black", fill = NA)
```
